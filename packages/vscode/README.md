...
- **One live indicator, in the reply itself.** The header of the answer being written says what the run is doing — `thinking`, `writing`, `working`, `2 agents`, or `waiting for you` when it is blocked — and holds the clock.
...
- **Usable without a mouse.** The chat list is keyboard-navigable, the transcript is a `log` and the live indicator a `status`, and nothing conveys state by colour alone.
